...
static jstring WhisperGGML_infer(JNIEnv *env, jobject instance, jlong handle, jfloatArray samples_array, jstring prompt, jobjectArray languages, jobjectArray bail_languages, jint decoding_mode, jboolean suppress_non_speech_tokens) {
    ...
    // Reduce per-segment decode budget to improve latency while preserving quality
    wparams.max_tokens = 128;
    wparams.n_threads = (int)num_procs;

    // Keep audio context tightly bound to input size; cap conservatively for mobile
    int computed_ctx = (int)ceil((double)num_samples / (double)(320.0)) + 32; // ~ 10 ms hop size
    wparams.audio_ctx = std::min(1200, computed_ctx);

    // Favor faster decoding behavior without changing temperature
    wparams.temperature_inc = 0.0f;
    wparams.no_context = true;       // do not rely on past text context
    wparams.n_max_text_ctx = 0;      // explicitly disable past text prompt accumulation
    // Single segment helps reduce overhead on short (<20s) inputs; keep disabled for longer inputs
    wparams.single_segment = num_samples <= 16000 * 20;
...
int register_WhisperGGML(JNIEnv *env) {
    const char *const kClassPathName = "com/slumdog88/dictationkeyboardai/offline/whisper/WhisperGGML";
