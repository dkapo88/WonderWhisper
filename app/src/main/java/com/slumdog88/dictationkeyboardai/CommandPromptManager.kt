...
/**
 * Manager class for handling command prompts storage and retrieval
 */
class CommandPromptManager(private val context: Context) {
    
    private val prefs: SharedPreferences = context.getSharedPreferences("app_settings", Context.MODE_PRIVATE)
    private val gson = Gson()
    
    companion object {
        private const val USER_PROMPTS_KEY = "user_command_prompts"
        private const val MIGRATION_V2_COMPLETE_KEY = "command_prompts_migration_v2_complete"
    }
    
    /**
     * Gets all available command prompts (default + user-created)
     */
    fun getAllPrompts(): List<CommandPrompt> {
        // Run migration if needed
        migrateOldDefaultPromptsIfNeeded()
        
        val defaultPrompts = CommandPrompt.getDefaultPrompts()
    ...
    /**
     * Gets only the default command prompts
     */
    fun getDefaultPrompts(): List<CommandPrompt> {
        return CommandPrompt.getDefaultPrompts()
    }
    
    /**
     * Gets user-created command prompts
     */
    fun getUserPrompts(): List<CommandPrompt> {
        val promptsJson = prefs.getString(USER_PROMPTS_KEY, "[]") ?: "[]"
        return try {
            val type = object : TypeToken<List<CommandPrompt>>() {}.type
            gson.fromJson(promptsJson, type) ?: emptyList()
        } catch (e: Exception) {
            Log.e("CommandPromptManager", "Error parsing user prompts from storage", e)
            emptyList()
        }
    }
    
    /**
     * Saves a user-created command prompt
     */
    fun saveUserPrompt(prompt: CommandPrompt) {
        ...
        Log.d("CommandPromptManager", "User prompt saved: ${prompt.name}")
    }
    
    /**
     * Deletes a user-created command prompt
     */
    fun deleteUserPrompt(promptId: String) {
        ...
        if (removedPrompt != null) {
            ...
            Log.d("CommandPromptManager", "User prompt deleted: ${removedPrompt.name}")
        }
    }
     ...
     */
    fun getPromptById(promptId: String): CommandPrompt? {
    ...
    fun renameUserPrompt(promptId: String, newName: String): Boolean {
        ...
        if (promptIndex >= 0) {
            ...
            Log.d("CommandPromptManager", "User prompt renamed from '${oldPrompt.name}' to '$newName'")
            return true
        }
    ...
    /**
     * Updates the content of a user-created command prompt
     */
    fun updateUserPromptContent(promptId: String, newContent: String): Boolean {
        ...
        if (textValidationError != null) {
            Log.w("CommandPromptManager", "Cannot update prompt content: $textValidationError")
            return false
        }
        ...
        if (promptIndex >= 0) {
            ...
            Log.d("CommandPromptManager", "User prompt content updated: ${oldPrompt.name}")
            return true
        }
     ...
     */
    fun createLegacyPrompt(promptText: String): CommandPrompt {
        return CommandPrompt(
            id = "legacy_command_${System.currentTimeMillis()}",
            name = "Legacy Command",
    ...
    private fun migrateOldDefaultPromptsIfNeeded() {
        ...
        Log.d("CommandPromptManager", "Running migration: converting old default prompts to user examples")
        ...
        val oldDefaultPrompts = CommandPrompt.getOldDefaultPromptsForMigration()
        ...
        if (promptsToAdd.isNotEmpty()) {
            ...
            Log.d("CommandPromptManager", "Migrated ${promptsToAdd.size} old default prompts to user examples")
        }
        
        // Mark migration as complete
        prefs.edit().putBoolean(MIGRATION_V2_COMPLETE_KEY, true).apply()
        Log.d("CommandPromptManager", "Migration v2 completed")
    }
}